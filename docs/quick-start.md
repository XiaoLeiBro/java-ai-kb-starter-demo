...
## 2. 获取大模型 API Key（v0.2 起需要）

当前 `v0.1` 只包含注册、登录和 JWT 鉴权，不需要真实大模型 API Key。

从 `v0.2` 的知识库 RAG 主流程开始，项目会使用 **OpenAI Compatible API**。你可以选择任意兼容 OpenAI 接口的大模型服务。
...
编辑 `application-dev.yml`，至少修改 JWT Secret。模型 API Key 可先保留占位，等 v0.2 接入 RAG 主流程时再填真实值：
...
接入 v0.2 RAG 主流程后，建议用环境变量传入 API Key：
...
A：这是 v0.2 RAG 主流程接入模型后的问题。通常是以下原因：
...
A：这是 v0.2 RAG 主流程接入 Embedding 后的问题。Embedding 模型不同，向量维度可能不同。  
需要同步修改：
