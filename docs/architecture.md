...
已实现：知识库创建 / 列表、Markdown/TXT/文本型 PDF 上传、本地文件存储、文本切分、Embedding、pgvector 写入与检索、单轮 RAG 问答
已实现：对话会话创建 / 列表 / 详情 / 归档、会话消息历史、基础 LLM 调用记录查询
已实现：前端基础控制台、自然中文回答展示、引用来源展示、原文件下载
已预留：billing 限界上下文后续扩展边界
未实现：扫描件 OCR、Word/Excel/PPT 解析、文档删除与向量清理、异步索引、完整 Token 计费、成本报表、管理后台
```

本文档描述的是免费 Demo 的目标架构。当前 `v0.4` 已落地 RAG 主流程、文本型 PDF、原文件下载、对话历史和基础调用记录，商业版治理能力仍保持边界外。
