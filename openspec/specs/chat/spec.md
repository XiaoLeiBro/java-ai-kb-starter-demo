...
系统应支持创建、查看、列出和归档与知识库绑定且属于特定用户的会话。
...
#### 场景：创建会话，knowledgeBaseId 不存在

- **Given** 用户已登录
- **When** 用户请求创建对话会话，提供不存在的 `knowledgeBaseId`
- **Then** 系统返回 404

#### 场景：创建会话，knowledgeBaseId 属于其他用户
...
#### 场景：按 knowledgeBaseId 过滤会话，无匹配结果
...
系统应支持在会话中追加和获取消息。消息的可见性由所属会话的状态控制。
...
问答接口应接受可选的 `conversationId` 参数。提供该参数时，系统应校验所有权、状态和知识库一致性，然后自动持久化用户消息和助手消息，并记录调用日志。
